"use strict";
var charMapping = require('./charMapping');
var CHAR_TO_NIBBLE_MAP = (function () {
    var builder = charMapping.createBuilder();
...
var tryParseByte = function (char1, char2) {
...
var converter = {
     ...
     */
    toByte: function (char1, char2) {
        var byte = tryParseByte(char1, char2);
        if (undefined === byte)
            throw Error("unrecognized hex char '".concat(char1).concat(char2, "'"));
        return byte;
    },
    /**
     ...
     */
    hexToUint8: function (input) {
        if (0 !== input.length % 2)
            throw Error("hex string has unexpected size '".concat(input.length, "'"));
     ...
     */
    bytesToInt: function (input, size, isSigned) {
        // const mapping = {
        // 	false: {
        // 		1: Uint8Array,
        // 		2: Uint16Array,
        ...
        var view = new DataView(input.buffer, input.byteOffset);
        var mapping = {
            false: {
                1: view.getUint8,
                2: view.getUint16,
        ...
        var reader = mapping[isSigned][size];
        return reader.call(view, 0, true);
    },
    intToBytes: function (value, byteSize, isSigned) {
        if (isSigned === void 0) { isSigned = false; }
        var mapping = {
            false: {
                1: Uint8Array,
        ...
        var DataType = mapping[isSigned][byteSize];
        var typedBuffer = new DataType([value]);
