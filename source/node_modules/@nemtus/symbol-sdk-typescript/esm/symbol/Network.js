...
class Network extends BasicNetwork {
    ...
    constructor(name, identifier, epochTime, generationHashSeed = undefined) {
        super(name, identifier, new NetworkTimestampDatetimeConverter(epochTime, 'milliseconds'), () => sha3_256.create(), (addressWithoutChecksum, checksum) => new Address(new Uint8Array([...addressWithoutChecksum, ...checksum.subarray(0, 3)])), Address, NetworkTimestamp);
        this.generationHashSeed = generationHashSeed;
    }
}
Network.MAINNET = new Network('mainnet', 0x68, new Date(Date.UTC(2021, 2, 16, 0, 6, 25)), new Hash256('57F7DA205008026C776CB6AED843393F04CD458E0AA2D9F1D5F31A402072B2D6'));
Network.TESTNET = new Network('testnet', 0x98, new Date(Date.UTC(2021, 10, 25, 14, 0, 47)), new Hash256('7FCCD304802016BEBBCD342A332F91FF1F3BB5E902988B352697BE245F48E836'));
